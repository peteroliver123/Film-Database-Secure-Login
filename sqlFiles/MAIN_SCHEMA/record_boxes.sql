CREATE DATABASE  IF NOT EXISTS `record_boxes2` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `record_boxes2`;
-- MySQL dump 10.13  Distrib 8.0.38, for Win64 (x86_64)
--
-- Host: localhost    Database: record_boxes2
-- ------------------------------------------------------
...
CREATE TABLE `kitchen` (
  ...
  `age_rating` varchar(10) NOT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `check_kitchen` CHECK (((`age_rating` in ('U', 'PG', '12', '12A', '15', '18', 'BLANK')) and (`location` = 'kitchen')))
...
CREATE TABLE `lounge` (
  ...
  `age_rating` varchar(10) NOT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `check_lounge` CHECK (((`age_rating` in ( 'U', 'PG', '12', '12A', '15', '18', 'BLANK')) and (`location` =  'lounge')))
...
INSERT INTO `passwords` VALUES ('ADMIN','$2b$12$AeLBQHGblaDUA8kOFHPjtuRdHClKAB1C2x9Epl7a.6BeFMFP9sYV.','UUUBEIBKDIXFGN6CV7VUDAIGYX6MJDDF');
...
CREATE TABLE `users` (
  ...
  PRIMARY KEY (`username`),
  UNIQUE KEY `username_UNIQUE` (`username`),
  CONSTRAINT `check_admin` CHECK ((((`username` =  'ADMIN') and (`is_admin` = true)) or (`is_admin` = false))),
  CONSTRAINT `check_dates` CHECK (((`is_locked` = true) or (`date_created` = `date_unlock`))),
  CONSTRAINT `check_failed_entry` CHECK (((`failed_entry` >= 0) and (`failed_entry` <= 3))),
  CONSTRAINT `check_is_locked` CHECK (((`is_locked` = true) or (`is_locked` = false))),
...
INSERT INTO `users` VALUES ('ADMIN','2025-08-21 13:25:21.202163',0,'2025-08-21 13:25:21.202163',1,1,1),('sally','2025-08-21 13:25:21.202163',0,'2025-08-21 13:25:21.202163',0,0,0);
...
-- Dump completed on 2025-08-28  8:58:49
